...
export default function LoadingTypeSelect({ onValueChange }) {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          "http://132.148.79.178/api/v1/loadingType/",
          {
            headers: {
              Authorization:
  ...
  const handleChangeloadingType = (selected) => {
    onValueChange(selected);
    console.log(selected);
  };

  return (
    <Listbox value={selected} onChange={handleChangeloadingType}>
      {({ open }) => (
        <>
